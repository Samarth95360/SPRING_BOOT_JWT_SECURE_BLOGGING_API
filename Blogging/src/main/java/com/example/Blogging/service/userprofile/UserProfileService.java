...
public class UserProfileService {
    ...
    @Autowired
    public UserProfileService(UserRepo userRepo, UserProfileRepo userProfileRepo, ImageProxy imageProxy){
        ...
        if(user.isPresent()){
            if(user.get().getUserProfile()!=null){
                if(user.get().getUserProfile().getImageId() == null){
                    ...
                    return new ResponseEntity<>("Failed try again after some time",HttpStatus.BAD_REQUEST);
                }
        ...
        if(user.isPresent()){
            if(user.get().getUserProfile()!=null){
                ...
                if (userProfile.getImageId() != null) {
                    ResponseEntity<Resource> responseEntity = imageProxy.getUserProfileImage(userProfile.getImageId());
